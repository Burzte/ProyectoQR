...
export class InicioSesionComponent  implements OnInit {
  ...
  async login(usuario: string, clave: string) {
    ...
    await this.authService.buscarBD(usuario, clave); // Intentar hacer login
    ...
    this.authService.isAuthenticated$.subscribe(isAuthenticated => {
      ...
      this.authService.usuarioCompleto$.subscribe(usuarioCompleto => {
        if (isAuthenticated) {
          ...
          if (usuarioCompleto.role === "Docente") {
            this.usuario = ''; // Limpiar el campo de usuario
            this.clave = ''; // Limpiar el campo de clave
            this.router.navigate(['/principal-docente']); // Redirigir al usuario docente si el login es exitoso
          }
          else{
            this.usuario = ''; // Limpiar el campo de usuario
            this.clave = ''; // Limpiar el campo de clave
